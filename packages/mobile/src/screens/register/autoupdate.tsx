...
export const AutoUpdateProgressScreen: FunctionComponent = observer(() => {
  ...
  const percent = (progress && progress.totalBytes != 0) ? (progress.receivedBytes * 1.0 / progress.totalBytes) : 0
    ...
    <View style={style.flatten(["background-color-background"])}>
      ...
      >
        ...
        >
          ...
          >
            <Text style={style.flatten(["color-white", "text-caption", "text-center"])} >
              {intl.formatMessage({ id: "auto-update.downloading" })}
            </Text>
            <Text style={style.flatten(["color-white", "text-caption", "text-left", "margin-top-8"])} >
              {received} MB / {total} MB
            </Text>
            <Progress.Bar progress={percent} width={null} height={8} color="rgba(11, 116, 229, 1)" unfilledColor="rgba(235, 235, 240, 1)" />
            <Text style={style.flatten(["color-white", "text-caption", "text-right"])} >
            ...
            status != CodePush.SyncStatus.UNKNOWN_ERROR ? null :
              (
                ...
                >
                  ...
                  <Text style={style.flatten(["color-gray-100", "text-caption", "text-left"])} >
